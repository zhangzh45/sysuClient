...
public class GetService {
	...
	public  Map<String,String>  getCallService(int appId){
	         ...
	         if(json.size()>0){
	           for(int i=0;i<json.size();i++){// 遍历 jsonarray 数组，把每一个对象转成 json 对象
	             ...
	             if(job.getInt("appid")==appId){
	            	 mp.put("type", job.getString("type"));
	...
	public void deleteService(int ServiceId,int userId) throws Exception {
			try
			{
				...
				call.addParameter("arg1",org.apache.axis.encoding.XMLType.XSD_INT,
						javax.xml.rpc.ParameterMode.IN);
				call.invoke(new Object[]{ServiceId,userId});
			}catch (Exception e) {
		 ...
		 try
		  {
	          ...
	          Service service = new Service();
	          Call call = (Call) service.createCall();
	          call.setTargetEndpointAddress("http://127.0.0.1:8080/demo/EmployeeServerInterfacePort?wsdl");
