...
public class ParameterDAO extends BaseHibernateDAO {
	...
	public Parameter findById(java.lang.Integer id) {
		...
		try {
			Parameter instance = (Parameter) getSession().get("dao.Parameter",
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Parameter instance) {
		...
		try {
			List results = getSession().createCriteria("dao.Parameter")
	...
	public Parameter merge(Parameter detachedInstance) {
		...
		try {
			Parameter result = (Parameter) getSession().merge(detachedInstance);
